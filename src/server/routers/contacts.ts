...
export const contactsRouter = router({
  ...
  createContact: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Generate embedding asynchronously to avoid blocking the UI
      generateEmbedding(`${fullLegalName} ${input.email_address} ${input.city} ${input.responsible_person}`, ctx.tenantId)
        .then(async (embedding) => {
          if (embedding && !isUsingFallback) {
            await pool.query("UPDATE core_registry_contacts SET embedding = $1 WHERE id_uuid = $2 AND (tenant_id = $3 OR tenant_id = '1')", [`[${embedding.join(',')}]`, id, ctx.tenantId]).catch(console.error);
          }
        })
        .catch(console.error);
  ...
  updateContact: protectedProcedure
    .input(ContactSchema.extend({ 
      ...
      last_updated_at_utc: z.string().or(z.date()).optional().nullable(),
      overwrite_concurrency: z.boolean().optional()
    }))
    .output(z.object({ success: z.boolean(), updated_at_utc: z.string().or(z.date()).nullable() }))
    .mutation(async ({ input, ctx }) => {
      input = cleanLigatureHacksFromValue(input);
      const { id_uuid, last_updated_at_utc, overwrite_concurrency, ...data } = input;
      const fullLegalName = `${data.first_name || ''} ${data.last_name}`.trim();
      
      // Generate embedding asynchronously
      generateEmbedding(`${fullLegalName} ${data.email_address} ${data.city} ${data.responsible_person}`, ctx.tenantId)
        .then(async (embedding) => {
          if (embedding && !isUsingFallback) {
            await pool.query("UPDATE core_registry_contacts SET embedding = $1 WHERE id_uuid = $2 AND (tenant_id = $3 OR tenant_id = '1')", [`[${embedding.join(',')}]`, id_uuid, ctx.tenantId]).catch(console.error);
          }
        })
        .catch(console.error);
      ...
      let wasDraft = false;
      let dbUpdatedAt: string | Date | null = null;

      if (isUsingFallback) {
        const found = fallbackStore.contacts.find(c => c.id_uuid === id_uuid);
        if (found) {
          if (found.is_verified_by_human === false) {
            wasDraft = true;
          }
          dbUpdatedAt = found.updated_at_utc || null;
        }
      } else {
        const checkRes = await pool.query(
          "SELECT is_verified_by_human, updated_at_utc FROM core_registry_contacts WHERE id_uuid = $1 AND (tenant_id = $2 OR tenant_id = '1')",
          [id_uuid, ctx.tenantId]
        );
        if (checkRes.rows.length > 0) {
          if (checkRes.rows[0].is_verified_by_human === false) {
            wasDraft = true;
          }
          dbUpdatedAt = checkRes.rows[0].updated_at_utc;
        }
      }

      // Concurrency check
      if (!overwrite_concurrency && last_updated_at_utc && dbUpdatedAt) {
        const dbTime = new Date(dbUpdatedAt).getTime();
        const clientTime = new Date(last_updated_at_utc).getTime();
      ...
      if (isUsingFallback) {
        ...
        return { success: true, updated_at_utc: fallbackTime };
      }
      const updateRes = await pool.query(`
        ...
        WHERE id_uuid = $33 AND (tenant_id = $34 OR tenant_id = '1')
      ...
      const newUpdatedAt = updateRes.rows[0]?.updated_at_utc;
      ...
      return { success: true, updated_at_utc: newUpdatedAt };
    }),
  ...
  importContacts: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      for (const rawItem of input) {
        ...
        // Generate embedding asynchronously
        generateEmbedding(`${fullLegalName} ${item.email_address || ''} ${item.city || ''} ${item.responsible_person || ''}`, ctx.tenantId)
          .then(async (embedding) => {
            if (embedding && !isUsingFallback) {
              await pool.query("UPDATE core_registry_contacts SET embedding = $1 WHERE id_uuid = $2", [`[${embedding.join(',')}]`, id]).catch(console.error);
            }
          })
          .catch(console.error);
