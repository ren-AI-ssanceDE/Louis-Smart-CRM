...
import { BaseInvoiceSchema, InvoiceSchema, InvoiceFullSchema } from "../../lib/schemas.js";
...
const sanitizeInputLigatures = <T>(obj: T): T => {
  ...
  if (typeof obj === "object" && obj !== null) {
    ...
    for (const key of Object.keys(record)) {
      newObj[key] = sanitizeInputLigatures(record[key]);
    }
...
export const invoicesRouter = router({
      ...
      try {
        if (isUsingFallback) {
          return (fallbackStore.invoices || []).map(inv => {
            ...
            return {
              ...inv,
              payment_status: inv.payment_status || inv.status || 'issued',
              status: inv.payment_status || inv.status || 'issued',
        ...
        const mapped = res.rows.map(row => {
          ...
          const dataToValidate = {
            ...r,
            payment_status: r.payment_status || r.status || 'issued',
            status: r.payment_status || r.status || 'issued',
            ai_confidence_score: confidence,
  ...
  finalizeDraft: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        workflowEventBus.emitEvent(ctx.tenantId, 'invoice.status_updated', {
          id_uuid,
          payment_status: 'issued',
          invoice_number: computedInvoiceNumber
        });
  ...
  updateInvoice: protectedProcedure
    .input(BaseInvoiceSchema.extend({
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (isUsingFallback) {
        ...
        workflowEventBus.emitEvent(ctx.tenantId, 'invoice.status_changed', {
          id_uuid,
  ...
  deleteInvoice: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!isDraft) {
        ...
        const latestInvoice = nonDraftInvoices.reduce<Invoice | null>((max, current) => {
