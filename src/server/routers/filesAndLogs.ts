...
export const filesAndLogsRouter = router({
  ...
  saveFile: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        // Trigger file uploaded event for CRM custom automated workflows or MCP integration safely
        workflowEventBus.emitEvent(ctx.tenantId, 'file.uploaded', {
          file_name: input.filename,
          file_size_bytes: buffer.length,
          entity_type: input.type,
          entity_id: input.id_uuid,
  ...
  getAuditLogs: adminProcedure
    ...
    .query(async ({ ctx, input }) => {
      const f = input;
      if (isUsingFallback) {
        let logs = (fallbackStore.auditLogs || []).filter(log => log.tenant_id === ctx.tenantId || log.tenant_id === '1');
        if (f.eventType) logs = logs.filter(l => l.event_type === f.eventType);
        if (f.actor) logs = logs.filter(l => (l.actor_identity || '').toLowerCase().includes((f.actor || '').toLowerCase()));
        if (f.entityType) logs = logs.filter(l => (l.entity_type || '').toLowerCase().includes((f.entityType || '').toLowerCase()));
        return logs.slice(f.offset || 0, (f.offset || 0) + (f.limit || 100));
      }
      const conditions: string[] = [`(tenant_id = $1 OR tenant_id = '1')`];
      const params: unknown[] = [ctx.tenantId];
      if (f.eventType) { params.push(f.eventType); conditions.push(`event_type = $${params.length}`); }
      if (f.actor) { params.push(`%${f.actor}%`); conditions.push(`LOWER(actor_identity) LIKE LOWER($${params.length})`); }
