...
export const companiesRouter = router({
  ...
  createCompany: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      generateEmbedding(`${input.full_legal_name} ${input.street} ${input.city} ${input.responsible_person}`, ctx.tenantId)
        .then(async (embedding) => {
          if (embedding && !isUsingFallback) {
            await pool.query("UPDATE core_registry_companies SET embedding = $1 WHERE id_uuid = $2 AND (tenant_id = $3 OR tenant_id = '1')", [`[${embedding.join(',')}]`, id, ctx.tenantId]).catch(console.error);
          }
        })
        .catch(console.error);
  ...
  updateCompany: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Generate embedding asynchronously
      generateEmbedding(`${data.full_legal_name} ${data.street} ${data.city} ${data.responsible_person}`, ctx.tenantId)
        .then(async (embedding) => {
          if (embedding && !isUsingFallback) {
            await pool.query("UPDATE core_registry_companies SET embedding = $1 WHERE id_uuid = $2 AND (tenant_id = $3 OR tenant_id = '1')", [`[${embedding.join(',')}]`, id_uuid, ctx.tenantId]).catch(console.error);
          }
        })
        .catch(console.error);
      ...
      if (isUsingFallback) {
        ...
        const checkRes = await pool.query(
          "SELECT is_verified_by_human, updated_at_utc FROM core_registry_companies WHERE id_uuid = $1 AND (tenant_id = $2 OR tenant_id = '1')",
      ...
      const updateRes = await pool.query(`
        ...
        WHERE id_uuid = $30 AND (tenant_id = $31 OR tenant_id = '1')
  ...
  importCompanies: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      for (const rawItem of input) {
        ...
        // Generate embedding asynchronously
        generateEmbedding(`${item.full_legal_name} ${item.street || ''} ${item.city || ''} ${item.responsible_person || ''}`, ctx.tenantId)
          .then(async (embedding) => {
            if (embedding && !isUsingFallback) {
              await pool.query("UPDATE core_registry_companies SET embedding = $1 WHERE id_uuid = $2", [`[${embedding.join(',')}]`, id]).catch(console.error);
            }
          })
          .catch(console.error);
