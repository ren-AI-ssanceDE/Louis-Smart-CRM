...
export async function initDatabase() {
  ...
  try {
    ...
    await pool.query(`
      ...
      -- additiv (additiv): MCP-Key-Tool-Berechtigungen (create/update/delete pro Tool)
      ALTER TABLE mcp_api_keys ADD COLUMN IF NOT EXISTS tool_permissions JSONB;

      -- Migration (additiv, idempotent): Bestands-Keys mit tool_permissions bekommen
    ...
    await pool.query(`
      ...
      -- additiv (additiv): Token-Zerlegung je LLM-Call — Anteile von
...
export async function recordAgentRun(run: {
  ...
  durationMs: number;
  activeTools: number;
  // Token-Zerlegung: Token-Zerlegung je LLM-Call (optional, additiv)
