...
import { McpApiKey, McpApiKeyScopeSchema, McpToolPermissionActionSchema } from "../../lib/schemas.js";
...
export function normalizeMcpApiKey(rawRow: unknown): McpApiKey {
  ...
  // Tool-Berechtigungen: tool_permissions durchreichen (JSONB kommt als Objekt, Fallback als Objekt)
  if (cleaned.tool_permissions && typeof cleaned.tool_permissions === "object") {
    cleaned.tool_permissions = cleaned.tool_permissions as Record<string, string[]>;
  } else {
    cleaned.tool_permissions = null;
  }
...
export async function generateMcpApiKey(params: {
  ...
  // Tool-Berechtigungen: Tool-Berechtigungen validieren (nur bekannte Aktionen)
  const toolPermissions = params.tool_permissions && typeof params.tool_permissions === "object"
    ? Object.fromEntries(
        Object.entries(params.tool_permissions)
          .filter(([, actions]) => Array.isArray(actions) && actions.length > 0)
          .map(([tool, actions]) => [
            tool,
            (actions as string[]).filter((a) => McpToolPermissionActionSchema.safeParse(a).success)
          ])
  ...
  if (isUsingFallback) {
    ...
    const query = `
      INSERT INTO mcp_api_keys (
        id_uuid, tenant_id, key_name, key_hash, key_prefix, scopes, tool_permissions,
        is_active, last_used_at, expires_at, created_by_user_id, created_at, updated_at
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, true, null, $8, $9, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      RETURNING *
    `;
    ...
    const res = await pool.query(query, [
      ...
      toolPermissions ? JSON.stringify(toolPermissions) : null,
      expiresAt,
      userId,
    ]);
