...
export const MCP_TOOL_ACTIONS: Record<string, "create" | "update" | "delete" | "approve" | "send" | "run"> = {
  // Unternehmen
  "crm_create_company": "create",
  "crm_update_company": "update",
  // Kontakte
  "crm_create_contact": "create",
  "crm_update_contact": "update",
  // Rechnungen
  "crm_create_invoice": "create",
  "crm_update_invoice_status": "update",
  "crm_generate_invoice_pdf": "update",
  // Angebote
  ...
  // Kanban
  "kanban_create_board": "create",
  "crm_create_kanban_card": "create",
  "kanban_update_card": "update",
  "crm_move_kanban_card": "update",
  "kanban_delete_card": "delete",
  // Notizen
  "notes_create": "create",
  "notes_update": "update",
  "notes_delete": "delete",
  ...
  "vault_update": "update",
  "vault_delete": "delete",
  "crm_upload_vault_document": "create",
  // Mail
  "mail_approve_draft": "approve",
  // Workflows
...
function checkMcpToolScope(name: string, ctx: McpContext): void {
 ...
 // neue Tool-Familien — Schreib-Tools
  ...
  if (isReadTool) {
    ...
    const toolPerms = (ctx?.keyInfo?.tool_permissions || null) as Record<string, string[]> | null;
    if (toolPerms && Object.keys(toolPerms).length > 0) {
      const action = getToolAction(name);
      if (action) {
        const allowed = toolPerms[name];
        if (!allowed || !allowed.includes(action)) {
          throw new Error(
            `Insufficient permissions: Aktion '${action}' ist für Tool '${name}' nicht freigegeben ` +
            `(erlaubt: ${allowed ? allowed.join(", ") : "keine"}).`
          );
        }
      }
      // Kein Matrix-Eintrag → generische Scope-Prüfung gilt (bereits oben bestanden)
    }
  }
}
