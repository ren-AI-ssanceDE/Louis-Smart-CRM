...
// FIX B--3 ( P0): Wandelt eine Klartext-Workflow-Instruktion in das
...
export function toNoteDraftJson(instruction: string, resolvedContactId?: string, resolvedCompanyId?: string): string {
  ...
  try {
    ...
    if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
      ...
      // Kompaktierung: aufgelöste Ziel-ID ergänzen, falls nicht in der Instruktion
  ...
  if (idField) {
    ...
    // Kompaktierung: aufgelöste Ziel-ID aus vorherigen Schritten (node_results) ergänzen
...
// FIX B--3 ( P0): Gemeinsamer Argument-Synthesizer für Workflow-
    ...
    schemaInstructions = `Generate a JSON object matching this schema:
{
  ...
  "line_items": [
    ...
    // Kompaktierung: Notiz-CREATE — Ziel-ID aus dem aufgelösten Kontext (resolved_contact_id_uuid)
    schemaInstructions = `Generate a JSON object matching this schema:
{
    ...
    schemaInstructions = `Generate a JSON object matching this schema:
{
  ...
  "labels": ["optional array of strings"]
    ...
    // FIX B--3 ( P0): Vault-Schreib-Tools — die Instruktion nennt Datei
    ...
    if (workflow.dag_structure) {
      ...
      if (isResuming) {
        ...
        if (inst) {
          ...
          // Kompaktierung: isWaitToolNode ergänzt — learn_workflow speichert WAIT als
 ...
 // 4A: Zusätzliche Wissens-/Vault-/Memory-Tools als Workflow-Schritte
          try {
            ...
            if (step.tool === "create_note_draft" || step.tool === "executeCreateNoteDraft") {
              ...
              // FIX B--3 ( P0): Instruktion NACH JSON wrappen — die Funktion
