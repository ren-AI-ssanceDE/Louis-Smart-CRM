...
// FIX B--3 ( P0): Erkennt WAIT-Schritte, die als ACTION-Tool gelernt wurden
// (learn_workflow speichert "wait"/"delay" als tool_identifier statt als WAIT-Knoten).
// Exportiert (Kompaktierung): workflowExecutor nutzt sie im Resume-Pfad (WAIT-Sprung).
...
// Kompaktierung: Ziel-Entität aus der natürlichen Sprache auflösen (Kontakt/Firma per
...
export async function resolveTargetEntityId(
  ...
  // Kompaktierung: Vorherige Schritt-Ergebnisse durchsuchen — wenn ein früherer Schritt
  ...
  if (emailMatch) {
    ...
    const m = text.match(/(?:am|an|dem|den)\s+([A-Za-zÀ-ÿ0-9][A-Za-zÀ-ÿ0-9 _-]{2,60})/i);
  ...
  try {
    if (hasContactRef && !hasCompanyRef) {
      if (isUsingFallback) {
        ...
        // Kompaktierung: Erst exakter Match, dann Token-Fallback (ein Wort des
  ...
  public async executeDAG(
    ...
    try {
      ...
      if (node.type === "CONDITIONAL") {
        ...
        // gelernt wurden (tool_identifier "wait"/"delay"/"executeWait", FIX B--3):
...
`;
 ...
 // 4B (Live-Bugfix): Generischer Tool-Dispatch für ALLE
      ...
      try {
        ...
        // FIX B--3 ( P0): Draft-/Schreib-Tools brauchen strukturierte JSON-
        ...
        if (needsSynthesizedArgs) {
          try {
            // Kompaktierung: Ziel-Entität aus natürlicher Sprache auflösen und in den
        ...
        if (toolName === "executeCrmDataAnalyst" || toolName === "crm_data_analyst") {
          ...
          // FIX B--3 ( P0): Instruktion in JSON wrappen (toNoteDraftJson) —
          ...
          // Kompaktierung: Zusätzlich die im Synthesizer-Kontext aufgelöste Ziel-ID
...
// : Workflow-Graph-Pfad schreibt direkt (bypassApproval=true)
          ...
          // FIX B--3 ( P0): synthetisierte JSON-Argumente statt Roh-Instruktion
