 ...
 */
export async function executeWebSearch(query: string, attempt: number = 1, tenantId: string = '1', aiClient?: GoogleGenAI): Promise<ToolResult<Record<string, unknown>>> {
  ...
  if (selectedEngine === 'google_custom_search') {
    if (!googleApiKey || !googleCx) {
      return createToolError("Google Custom Search JSON-Schnittstelle wurde ausgewählt, aber der Google API-Key oder die Custom Search Engine ID (CX) ist in den Einstellungen nicht konfiguriert.");
    }
    try {
      ...
      if (data && data.items && Array.isArray(data.items)) {
        const textContent = data.items
          ...
          .join("\n\n");
        return createToolSuccess({
          message: textContent || "Google Custom Search hat keine Ergebnisse geliefert.",
          engine: 'google_custom_search'
        });
      }
      return createToolSuccess({
        message: "Die Google Custom Search JSON-Schnittstelle hat keine Items zurückgegeben.",
        engine: 'google_custom_search'
      });
    } catch (gSearchErr) {
    ...
    try {
      ...
      if (data && data.results && Array.isArray(data.results)) {
        const textContent = data.results
          ...
          .join("\n\n");
        return createToolSuccess({
          message: textContent || "No results found on SearXNG.",
          engine: 'searxng'
        });
      }
      ...
      try {
        ...
        return createToolSuccess({
          message: textMatches.slice(0, 12).join("\n\n") || "SearXNG Scrape produced no text.",
          engine: 'searxng_scrape'
        });
      } catch (htmlErr) {
        return createToolError(`SearXNG web search failed: ${(htmlErr as Error).message}`);
      }
    }
  }
  ...
  try {
    ...
    return createToolError(`Web search failed after 4 attempts (Sperren/Captchas der Engine blockieren eventuell den Container): ${(err as Error).message}`);
  }
}
