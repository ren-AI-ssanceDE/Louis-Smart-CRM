 ...
 */
export async function executeTextGenerator(tenantId: string, instruction: string, aiClient?: GoogleGenAI): Promise<ToolResult<Record<string, unknown>>> {
  ...
  try {
    ...
    return createToolSuccess({
      text: res.text || "Kein Text generiert.",
      message: res.text || "Kein Text generiert."
    });
  } catch (err) {
    return createToolError(`Text-Generierung fehlgeschlagen: ${(err as Error).message}`);
  }
}
