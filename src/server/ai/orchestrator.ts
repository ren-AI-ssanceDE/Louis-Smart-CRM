...
// 061 P1-2: Deterministischer Workflow-Start — erkennt "Starte <Name>" /
// "Führe <Name> aus" und liefert den workflow_-Tool-Namen. Der User hat den
// Workflow explizit benannt → kein LLM-Entscheiden über die Tool-Wahl (sonst
// Varianz: Louis führt die Schritte teils einzeln aus statt workflow_X zu
// rufen → Drafts statt Persistenz). Rückgabe: workflow_-Name oder null.
export function detectWorkflowStartIntent(
  message: string,
  workflowNames: Array<string>
): string | null {
  const msg = String(message || "").trim();
  if (!msg || workflowNames.length === 0) return null;
  // "Starte X", "Führe X aus", "Führe den Workflow X aus", "Starte den Workflow X"
  const m = msg.match(/\b(?:starte|start|führe|fuehre|ausführen|ausfuehren|run|execut)\w*\s+(?:den\s+|den\s+workflow\s+|workflow\s+)?["']?([A-Za-z0-9][A-Za-z0-9 _\-]{1,80})["']?/i);
  if (!m) return null;
  const target = m[1].trim().toLowerCase();
  for (const name of workflowNames) {
    const normalized = String(name || "").toLowerCase();
    if (normalized === target) {
      return `workflow_${name.replace(/[^a-zA-Z0-9_]/g, "_")}`;
    }
    // Teil-/Kürzel-Match: "Onboarding" → "Kunden-Onboarding"
    if (normalized.toLowerCase().includes(target) || target.includes(normalized.toLowerCase())) {
      return `workflow_${name.replace(/[^a-zA-Z0-9_]/g, "_")}`;
    }
  }
  return null;
 ...
 * Lern-Absicht, damit learn_workflow als Ziel-Tool markiert werden kann.
 */
export function detectLearnWorkflowIntent(message: string): boolean {
  const msg = String(message || "").trim();
  if (!msg) return false;
  // Explizite Lern-/Anlege-Signale
  const learnSignal =
    /\b(neuer workflow|neue workflow|lerne|lerne .*workflow|lern .*workflow|merke dir (als )?workflow|merke dir .*workflow|merke .*workflow|als workflow (anlegen|speichern|merken)|workflow anlegen|workflow lernen|workflow speichern|speichere .*als workflow|speichere .*workflow)\b/i.test(msg);
  if (!learnSignal) return false;
  // Wenn-Dann-Logik (Trigger-Beschreibung) — erhöht Eindeutigkeit
  const whenThen =
    /\bwenn\b/i.test(msg) &&
