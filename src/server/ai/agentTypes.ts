...
export interface AgentPipelineContext {
 ...
 // Task 0: Admin-konfigurierbare ReAct-Laufzeitparameter (Regel 12)
  ...
  // Token-Zerlegung: Token-Zerlegung je Iteration (Schätzung) — für prompt_tokens_breakdown
  promptBreakdownSum?: number;
  promptBreakdownParts?: Array<{
    system_prompt: number; history: number; tool_schemas: number;
    tool_results: number; user_message: number; other: number; estimated_total: number;
  }>;
