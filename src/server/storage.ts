...
export function getEntityStoragePath(type: string, id: string, name: string, tenantId: string = '1') {
  ...
  const safeName = name.replace(/[^a-z0-9]/gi, '_').toLowerCase().replace(/_+$/, '') || 'entity';
...
import { generateEmbedding as aiGenerateEmbedding, formatVectorForPostgres } from "./ai/embeddingHelper.js";

export async function generateEmbedding(text: string, tenantId: string = '1') {
  try {
    return await aiGenerateEmbedding(text, tenantId);
  } catch (err) {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        if (isUsingFallback || !pool) {
          ...
          fallbackStore.louisAiKnowledgeChunks.push({
            ...
            associated_company_id: scope === 'company' ? associatedId : null,
            associated_contact_id: scope === 'contact' ? associatedId : null,
            document_type: mimeType.includes('pdf') ? 'pdf' : 'document',
          ...
          await pool.query(
            `INSERT INTO sys_louis_ai_knowledge_chunks (
              id_uuid, tenant_id, document_id, chunk_index, chunk_text, embedding, 
              scope, associated_company_id, associated_contact_id, document_type, needs_reembedding, 
              created_at_utc, updated_at_utc
             )
             VALUES ($1, $2, $3, $4, $5, $6::vector, $7, $8, $9, $10, FALSE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
            [
              chunkId, tenantId, docId, i, textChunk, vectorStr,
              scope, scope === 'company' ? associatedId : null, scope === 'contact' ? associatedId : null,
...
export async function forceManualIngest(
  ...
  try {
    ...
    // 1. Clean existing index for this document beforehand to avoid duplicates
  ...
  try {
    if (isUsingFallback || !pool) {
      ...
      if (res.rows.length > 0) {
        ...
        // Chunks zuerst löschen, sonst bleiben verwaiste Vektoren (Stale-Daten im RAG)
        await pool.query("DELETE FROM sys_louis_ai_knowledge_chunks WHERE document_id = $1", [docId]);
...
export async function syncVaultFilesToRag(tenantId: string = "1") {
  try {
    ...
    // Scan global knowledge data vault for current tenant
    const knowledgeTenantRoot = path.resolve(process.cwd(), "knowledge_data_vault", tenantId);
    if (fs.existsSync(knowledgeTenantRoot)) {
      const files = listDirsSafe(knowledgeTenantRoot);
      for (const filename of files) {
        if (filename.startsWith('.')) continue;
        const filePath = path.join(knowledgeTenantRoot, filename);
    ...
    // Scan global knowledge data vault for fallback tenant '1' if different
    if (tenantId !== "1") {
      const knowledgeFallbackRoot = path.resolve(process.cwd(), "knowledge_data_vault", "1");
      if (fs.existsSync(knowledgeFallbackRoot)) {
        const files = listDirsSafe(knowledgeFallbackRoot);
        for (const filename of files) {
          if (filename.startsWith('.')) continue;
          const filePath = path.join(knowledgeFallbackRoot, filename);
          if (fs.existsSync(filePath) && fs.statSync(filePath).isFile()) {
            await ingestFileToRag(filePath, filename, tenantId, "global");
          }
        }
    ...
    if (isUsingFallback || !pool) {
      ...
      await pool.query(
        `INSERT INTO sys_louis_ai_knowledge_chunks (
         ...
         VALUES ($1, $2, $3, $4, $5, $6::vector, 'global', NULL, NULL, 'email', FALSE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
        [chunkId, tenantId, docId, i, textChunk, vectorStr]
      );
    }
  }
    ...
    if (isUsingFallback || !pool) {
      ...
      await pool.query(
        `INSERT INTO sys_louis_ai_knowledge_chunks (
         ...
         VALUES ($1, $2, $3, $4, $5, $6::vector, $7, $8, $9, 'email', FALSE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
        [
          chunkId, tenantId, docId, i, textChunk, vectorStr,
          scope, scope === 'company' ? entityId : null, scope === 'contact' ? entityId : null
        ]
      );
    }
  }
...
export async function ingestEmailToRag(params: IngestEmailToRagParams): Promise<void> {
  // Fire and forget background execution to ensure snail-paced embedding generation (+ network requests to Gemini)
  // never block the high-priority SMTP mail delivery threads for manual or automated mails.
  Promise.resolve().then(async () => {
    ...
    try {
      ...
      // 4. Resolve exact list of attachment names as specified ("for attachments, the exact name of the attached file")
...
${attachmentsText}`;
      ...
      console.error(`[ingestEmailToRag Background Task] Execution failed:`, innerErr);
    }
  }).catch((outerErr) => {
    console.error(`[ingestEmailToRag Background Task] Thread initialization failed:`, outerErr);
  });
}
