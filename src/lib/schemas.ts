...
export const CustomWorkflowSchema = z.object({
  ...
  trigger_config: z.union([
    ...
    z.record(z.string(), z.unknown()).superRefine((val, ctx) => {
      // Alt-Records ohne conditions sind erlaubt; mit conditions → strikt
      if (val && typeof val === "object" && "conditions" in val) {
        ctx.addIssue({ code: "custom", message: "conditions nur in strukturierter Form erlaubt" });
      }
    }),
  ]).nullable().optional(),
...
// 061 P0-2: Tool-Berechtigungen pro MCP-Key — { "<tool_name>": ["create","update","delete"] }
