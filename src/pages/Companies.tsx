...
export const Companies = () => {
  const { t, i18n } = useTranslation(['companies', 'common', 'dashboard', 'offers', 'validation_errors']);
  ...
  const [pendingUpdateData, setPendingUpdateData] = React.useState<(z.infer<typeof CompanySchema> & { id_uuid: string }) | null>(null);
  ...
  const generateOfferPdfMutation = trpc.generateOfferPdf.useMutation({
    onSuccess: (data) => {
      toast.success(t('offers:toast_pdf_success', { defaultValue: 'PDF erfolgreich generiert!' }));
      utils.getOffers.invalidate();
      window.open(`/api/files/download?path=${encodeURIComponent(data.filePath)}`, '_blank');
    },
    onError: (err) => {
      toast.error(err.message || t('common:error', { defaultValue: 'Fehler' }));
    }
  });
  ...
  const { data: offers = [] } = trpc.getOffers.useQuery();
  ...
  const companyOffers = React.useMemo(() => {
    if (!selectedCompany) return [];
    return offers
      .filter(off => off.associated_company_id === selectedCompany.id_uuid)
      .sort((a, b) => (a.offer_number || '').localeCompare(b.offer_number || ''));
  }, [offers, selectedCompany]);
  ...
  const getColHeaderLabel = (colKey: string) => {
    switch (colKey) {
      ...
      case 'offer':
        return t('admin:columns_config.labels.offer');
      default:
        return 'Column';
    }
  };
      ...
      case 'offer':
        return (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              localStorage.setItem('open_create_offer_for_company_id', company.id_uuid);
              window.dispatchEvent(new CustomEvent('navigate-to-tab', { detail: 'offers' }));
            }}
            className="px-3 py-1.5 rounded bg-accent-blue/10 hover:bg-accent-blue text-accent-blue hover:text-white border border-accent-blue/25 font-bold text-[9px] uppercase tracking-wider transition-all duration-200 cursor-pointer active:scale-95 flex items-center gap-1.5"
          >
            <Plus size={10} />
  ...
  React.useEffect(() => {
    ...
    if (queryId && companies.length > 0) {
      ...
      const found = companies.find(c => c.id_uuid === queryId);
      if (found) {
  ...
  React.useEffect(() => {
    if (selectedCompany) {
      const updated = companies.find(c => c.id_uuid === selectedCompany.id_uuid);
      if (updated && (
        updated.updated_at_utc !== selectedCompany.updated_at_utc ||
  ...
  const updateCompanyMutation = trpc.updateCompany.useMutation({
    onSuccess: (data, variables) => {
      if (selectedCompany && variables) {
        const vars = variables as (z.infer<typeof CompanySchema> & { id_uuid: string });
        setSelectedCompany(prev => prev ? { 
          ...prev, 
          ...vars,
          updated_at_utc: data.updated_at_utc || prev.updated_at_utc
        } : null);
      }
      setIsDialogOpen(false);
    ...
    try {
      ...
      if (selectedCompany) {
        updateCompanyMutation.mutate({ 
          ...validatedData, 
          id_uuid: selectedCompany.id_uuid,
          last_updated_at_utc: selectedCompany.updated_at_utc
        });
      } else {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      {isLoading ? (
        ...
        <div className="bg-primary-light/30 border border-white/5 rounded-xl shadow-2xl overflow-hidden backdrop-blur-sm">
          <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
            <table className="w-full min-w-[850px] text-left border-collapse">
              <thead>
                <tr className="bg-primary-light/50 border-b border-white/5">
                  <th className="px-4 sm:px-8 py-4 sm:py-6 text-[10px] font-bold text-slate-500 uppercase tracking-widest whitespace-nowrap">{t('table.identity')}</th>
                  {displayColumns.map((colKey, colIdx) => (
                    <th key={`th-${colKey}-${colIdx}`} className="px-4 sm:px-8 py-4 sm:py-6 text-[10px] font-bold text-slate-500 uppercase tracking-widest whitespace-nowrap">
              ...
              <tbody className="divide-y divide-white/5">
                {paginatedCompanies.map((company, idx) => (
                  ...
                  >
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                          ...
                          {company.is_verified_by_human === false && (
                            <span className="inline-flex items-center gap-1 bg-amber-500/10 text-amber-400 text-[10px] font-extrabold uppercase px-2 py-0.5 rounded border border-amber-500/20 tracking-wider w-fit">
                              {t('companies:draft', { defaultValue: 'Entwurf' })}
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    {displayColumns.map((colKey, colIdx) => (
                      <td key={`td-${company.id_uuid}-${colKey}-${colIdx}`} className="px-4 sm:px-8 py-4 sm:py-6 whitespace-nowrap">
                ...
                <div className="lg:col-span-2 p-12 space-y-12 overflow-y-auto">
                  ...
                  <div className="space-y-8">
                    ...
                    {selectedCompany && (
                      ...
                      <div className="pt-8 border-t border-white/5 space-y-6">
                        ...
                        {companyInvoices.length === 0 ? (
                          <div className="bg-primary-dark/30 border border-white/5 rounded-xl p-8 text-center text-xs text-slate-500 italic">
                            {t('companies:no_invoices_found_for_company')}
                          </div>
                        ) : (
                      ...
                      {/* Independent Angebotsverlauf Section */}
                      <div className="pt-8 border-t border-white/5 space-y-6">
                        <div className="flex items-center justify-between">
                          <h5 className="text-xs font-black text-white uppercase tracking-[0.2em] font-display flex items-center gap-2.5">
                            <FileText size={14} className="text-accent-orange" />
                            Angebotsverlauf ({companyOffers.length})
                          </h5>
                          <button
                            type="button"
                            onClick={() => {
                              localStorage.setItem('open_create_offer_for_company_id', selectedCompany.id_uuid);
                              window.dispatchEvent(new CustomEvent('navigate-to-tab', { detail: 'offers' }));
                              setIsDialogOpen(false);
                            }}
                            className="px-3 py-2 bg-accent-orange/10 hover:bg-accent-orange text-accent-orange hover:text-black border border-accent-orange/20 rounded-xl font-bold text-[10px] uppercase tracking-wider transition-all cursor-pointer flex items-center gap-2 active:scale-95 text-center"
                          >
                            <Plus size={12} />
                            {t('offers:create_offer', { defaultValue: 'Angebot erstellen' })}
                          </button>
                        </div>

                        {companyOffers.length === 0 ? (
                          <div className="bg-primary-dark/30 border border-white/5 rounded-xl p-8 text-center text-xs text-slate-500 italic">
                            {t('offers:no_offers_found_for_company', { defaultValue: 'Keine Angebote für dieses Unternehmen gefunden.' })}
                          </div>
                        ) : (
                          <div className="bg-primary-dark/40 border border-white/5 rounded-xl overflow-hidden divide-y divide-white/5 max-h-[300px] overflow-y-auto no-scrollbar">
                            {companyOffers.map((off) => {
                              const badgeClasses = cn(
                                "px-2 py-0.5 rounded text-[8px] font-black uppercase border tracking-wider",
                                off.offer_status === "draft" && "bg-neutral-500/10 text-neutral-400 border-neutral-500/20",
                                off.offer_status === "not_sent" && "bg-slate-500/10 text-slate-400 border-slate-500/20",
                                off.offer_status === "sent" && "bg-blue-500/10 text-blue-400 border-blue-500/20",
                                off.offer_status === "accepted" && "bg-emerald-500/10 text-emerald-400 border-emerald-500/20",
                                off.offer_status === "declined" && "bg-red-500/10 text-red-400 border-red-500/20"
                              );

                              return (
                                <div key={off.id_uuid} className="p-4 hover:bg-white/[0.02] transition-colors flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                  <div className="flex items-center gap-3">
                                    <div className="w-9 h-9 rounded-lg bg-primary-light border border-white/5 flex items-center justify-center text-slate-400">
                                      <FileText size={18} />
                                    </div>
                                    <div>
                                      <div className="text-white text-xs font-bold font-mono flex items-center gap-1.5">
                                        <span>{off.offer_number}</span>
                                        <span className={badgeClasses}>
                                          {t(`offers:${off.offer_status}`, { defaultValue: off.offer_status })}
                                        </span>
                                      </div>
                                      ...
                                      <div className="text-[9px] text-slate-500 font-semibold uppercase tracking-wider mt-0.5">
                                        {new Date(off.issue_date).toLocaleDateString(i18n.language)}
                                      </div>
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex items-center justify-between sm:justify-end gap-6 shrink-0">
                                    <div className="text-right">
                                      <div className="text-white text-xs font-bold font-mono">
                                        {formatCurrency(off.total_gross_amount || 0, off.currency_code)}
                                      </div>
                                      <div className="text-[9px] text-slate-500 font-semibold uppercase tracking-wider">
                                        {t('invoices:gross_sum', { defaultValue: 'Brutto' })}
                                      </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-1.5">
                                      <button
                                        type="button"
                                        onClick={() => generateOfferPdfMutation.mutate({ id_uuid: off.id_uuid })}
                                        disabled={generateOfferPdfMutation.isPending}
                                        className="p-2 bg-primary-light border border-white/5 rounded-lg text-slate-400 hover:text-white hover:bg-white/5 transition-all cursor-pointer active:scale-95 disabled:opacity-50"
                                        title={t('offers:generate_pdf', { defaultValue: 'PDF herunterladen/generieren' })}
                                      >
                                        <Download size={14} />
                                      </button>
                                      <button
                                        type="button"
                                        onClick={() => {
                                          localStorage.setItem('open_offer_id', off.id_uuid);
                                          window.dispatchEvent(new CustomEvent('navigate-to-tab', { detail: 'offers' }));
                                          setIsDialogOpen(false);
                                        }}
                                        className="p-2 bg-primary-light border border-white/5 rounded-lg text-accent-blue hover:bg-accent-blue hover:text-white transition-all cursor-pointer active:scale-95"
                                        title={t('offers:open_in_overview_action', { defaultValue: 'In Angebotsübersicht öffnen' })}
                                      >
                                        <ExternalLink size={14} />
                                      </button>
                                    </div>
      ...
      <Dialog
        ...
        title={t('companies:concurrency_conflict_title', { defaultValue: 'Bearbeitungskonflikt' })}
        size="md"
      >
        <div className="space-y-6 text-slate-300 font-semibold leading-relaxed font-body">
          <p className="text-sm">
            {t('companies:concurrency_conflict_desc', { defaultValue: 'Dieser Datensatz wurde in der Zwischenzeit von einem anderen Administrator geändert. Bitte laden Sie die Daten neu, um ein unabsichtliches Überschreiben zu verhindern.' })}
          </p>
          ...
          <div className="flex gap-4 pt-4 border-t border-white/5">
            <button
              ...
              onClick={() => {
                ...
                setSelectedCompany(null);
                utils.getCompanies.invalidate();
                toast.info(t('common:last_update', { defaultValue: 'Daten wurden aktualisiert' }));
              }}
              className="flex-1 bg-accent-blue hover:bg-accent-blue/80 text-white py-3.5 rounded-xl font-bold uppercase text-[10px] tracking-widest transition-all text-center cursor-pointer active:scale-95"
            >
              {t('companies:concurrency_conflict_reload', { defaultValue: 'Daten neu laden' })}
            </button>
            <button
              type="button"
              onClick={() => {
                if (pendingUpdateData) {
                  updateCompanyMutation.mutate({
                    ...pendingUpdateData,
                    overwrite_concurrency: true
                  });
                }
                setIsConflictOpen(false);
                setPendingUpdateData(null);
              }}
              className="flex-1 bg-red-600 hover:bg-red-700 text-white py-3.5 rounded-xl font-bold uppercase text-[10px] tracking-widest transition-all text-center cursor-pointer active:scale-95"
            >
              {t('companies:concurrency_conflict_overwrite', { defaultValue: 'Trotzdem überschreiben' })}
            </button>
          </div>
        </div>
      </Dialog>
