...
export const EmailDraftsApprovalPanel: React.FC = () => {
  ...
  const approveMutation = trpc.approve.useMutation({
    onSuccess: () => {
      toast.success(t('admin:mail.toast_approve_success', { defaultValue: 'E-Mail-Entwurf freigegeben und gesendet!' }));
      refetch();
  ...
  const rejectMutation = trpc.reject.useMutation({
    onSuccess: () => {
      toast.success(t('admin:mail.toast_reject_success', { defaultValue: 'E-Mail-Entwurf abgelehnt. Der Workflow-Zweig wurde abgebrochen.' }));
      refetch();
      utils.getWorkflowInstancesLog.invalidate();
    },
    onError: (err) => {
  ...
  const handleSaveEdit = (id: string) => {
    if (!editSubject.trim() || !editBody.trim()) {
      toast.warning(t('admin:mail.toast_empty_warning', { defaultValue: 'Betreff und Text dürfen nicht leer sein!' }));
      return;
    }
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className="text-base font-black text-white uppercase tracking-wider font-display">
              {t('admin:mail.draft_approvals_title', { defaultValue: 'E-Mail Entwurfsfreigaben' })}
            </h4>
            <p className="text-slate-500 text-[10px] uppercase font-bold tracking-widest mt-0.5">
              {t('admin:mail.draft_approvals_subtitle', { defaultValue: 'Sichte, editiere und verwalte die durch LOUIS-Workflows generierten E-Mails vor dem Versand.' })}
            </p>
          </div>
        </div>
        <button
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 bg-primary-light/10 border border-white/5 rounded-2xl">
          ...
          <span className="text-xs font-mono text-slate-500 uppercase tracking-widest">{t('admin:mail.loading_drafts', { defaultValue: 'Lade ausstehende Entwürfe...' })}</span>
        </div>
      ) : drafts.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 text-center bg-primary-light/10 border border-white/5 rounded-2xl py-20">
          ...
          <p className="text-sm font-bold text-slate-300">
            {t('admin:mail.no_pending_drafts', { defaultValue: 'Keine ausstehenden E-Mail-Freigaben' })}
          </p>
          <p className="text-xs text-slate-500 max-w-sm mt-1 mx-auto leading-relaxed">
            {t('admin:mail.no_pending_drafts_desc', { defaultValue: 'Aktuell liegen keine unbestätigten E-Mail Entwürfe vor. Falls Workflows mit Freigabepflicht ausgeführt werden, erscheinen diese direkt an dieser Stelle.' })}
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {(drafts as unknown as MailDraft[]).map((draft: MailDraft) => {
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-black text-white font-mono">{draft.recipient || t('common:not_assigned')}</span>
                      ...
                      <p className="text-[9px] text-slate-500 font-mono mt-0.5">
                        Draft ID: {draft.id_uuid.substring(0, 8)}... • {t('common:created')}: {new Date(draft.created_at_utc).toLocaleString()}
                      </p>
                    </div>
                  </div>
                  ...
                  {isEditing ? (
                    <div className="space-y-4">
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">{t('common:fields.subject', { defaultValue: 'Betreff' })}</label>
                        <input
                          type="text"
                          value={editSubject}
                      ...
                      {/* Msg Body area */}
                      <div className="space-y-1.5">
                        <label className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">{t('admin:mail.message_content', { defaultValue: 'Nachrichteninhalt' })} / {t('admin:mail.email_text', { defaultValue: 'E-Mail Text' })}</label>
                        <textarea
                          rows={10}
                          value={editBody}
                    ...
                    <div className="space-y-3">
                      <div>
                        <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">{t('common:fields.subject', { defaultValue: 'Betreff' })}</span>
                        <div className="text-white text-sm font-bold bg-primary-dark/30 px-3 py-2 rounded-lg border border-white/[0.03]">
                          {draft.subject || t('common:fields.no_subject', { defaultValue: 'Kein Betreff' })}
                        </div>
                      </div>

                      <div>
                        <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">{t('admin:mail.message_content', { defaultValue: 'Nachrichteninhalt' })}</span>
