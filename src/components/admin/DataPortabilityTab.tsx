...
import { 
  ...
  InvoiceTextTemplate, 
  InvoiceItemTemplate,
  OfferTextTemplate as OfferTemplate
} from '../../types';
import { CompanySchema, ContactSchema, EmailTemplateSchema, SignatureSchema, InvoiceTextTemplateSchema, InvoiceItemTemplateSchema, CompanyInput, ContactInput } from '../../lib/schemas';
...
export const DataPortabilityTab = () => {
  ...
  const [activePortabilityTab, setActivePortabilityTab] = useState<'stammdaten' | 'vorlagen' | 'rechnungstexte' | 'rechnungsposten' | 'angebotstexte'>('stammdaten');
  ...
  const [dragActiveOfferTemplate, setDragActiveOfferTemplate] = useState(false);
  ...
  const [offerTemplateStatus, setOfferTemplateStatus] = useState<{ type: 'idle' | 'success' | 'error'; message: string }>({ type: 'idle', message: '' });
  ...
  const [safeCompanies, setSafeCompanies] = useState<CompanyInput[]>([]);
  
  const [contactConflicts, setContactConflicts] = useState<ContactConflict[]>([]);
  const [safeContacts, setSafeContacts] = useState<ContactInput[]>([]);
  ...
  const { data: contacts = [], refetch: refetchContacts } = trpc.getContacts.useQuery();
  ...
  const { data: offerTemplates = [], refetch: refetchOfferTemplates } = trpc.getTemplates.useQuery();
  ...
  const importOfferTemplatesMutation = trpc.importOfferTemplates.useMutation();
  ...
  const handleDownloadTemplateEmailTemplates = () => {
    const headers = ['id_uuid', 'template_name_text', 'email_subject_text', 'email_body_content'];
    const example = ['', t('admin:data_portability.template_greeting_default', { defaultValue: 'Standard Begrueßung' }), 'Willkommen bei Louis Smart CRM', 'Hallo {{name}},\nvielen Dank fuer Ihre Nachricht!'];
  ...
  const handleDownloadTemplateSignatures = () => {
    const headers = ['id_uuid', 'signature_name_text', 'signature_body_content', 'is_default_signature'];
    const example = ['', t('admin:data_portability.template_signature_default', { defaultValue: 'Standard Signatur' }), 'Mit freundlichen Grueßen\nLouis Smart CRM Team', 'true'];
    const csv = generateCSV(headers, [example]);
  ...
  const handleDownloadTemplateInvoiceTextTemplates = () => {
    const headers = ['id_uuid', 'template_name_text', 'template_type_code', 'template_body_content'];
    const example = ['', t('admin:data_portability.template_intro_default', { defaultValue: 'Standard Einleitung' }), 'introductory', 'vielen Dank fuer die gute Zusammenarbeit. Hier ist Ihre Rechnung:'];
    const csv = generateCSV(headers, [example]);
  ...
  const handleExportInvoiceItemTemplates = () => {
    const headers = ['id_uuid', 'template_name_text', 'description', 'quantity', 'unit_price', 'vat_rate', 'unit_code', 'usage_scope'];
  ...
  const handleDownloadTemplateInvoiceItemTemplates = () => {
    const headers = ['id_uuid', 'template_name_text', 'description', 'quantity', 'unit_price', 'vat_rate', 'unit_code', 'usage_scope'];
    const example = ['', 'Entwicklungsstunde', 'Softwareentwicklung und Beratung', '1', '120.00', '19', 'HUR', 'both'];
    const csv = generateCSV(headers, [example]);
    triggerDownload('invoice_item_templates_template.csv', csv);
  };

  // OFFER TEXT TEMPLATE CSV EXPORT
  const handleExportOfferTemplates = () => {
    const headers = ['id_uuid', 'template_name_text', 'template_type_code', 'template_body_content'];
    const rows = offerTemplates.map(t => [
      t.id_uuid || '',
      t.template_name_text || '',
      t.template_type_code || '',
      t.template_body_content || ''
    ...
    const csv = generateCSV(headers, rows);
    triggerDownload('offer_text_templates_export.csv', csv);
  };

  const handleDownloadTemplateOfferTemplates = () => {
    const headers = ['id_uuid', 'template_name_text', 'template_type_code', 'template_body_content'];
    const example = ['', t('admin:data_portability.template_intro_default', { defaultValue: 'Standard Einleitung' }), 'introductory', 'vielen Dank fuer das Interesse. Hier ist Ihr Angebot:'];
    const csv = generateCSV(headers, [example]);
    triggerDownload('offer_text_templates_template.csv', csv);
  };
    ...
    try {
      ...
      const parsed = dataRows.map((row) => {
        ...
        if (!parseResult.success) {
          throw new Error(t('admin:data_portability.invalid_row_email_template', {
            defaultValue: 'Ungültige Daten in Zeile für E-Mail-Vorlage: {{details}}',
  ...
  const processSignatureCSV = async (text: string) => {
    try {
      ...
      const parsed = dataRows.map((row) => {
        ...
        if (!parseResult.success) {
          throw new Error(t('admin:data_portability.invalid_row_signature', {
            defaultValue: 'Ungültige Daten in Zeile für Signatur: {{details}}',
            details: parseResult.error.issues.map(e => `${e.path.join('.')}: ${e.message}`).join(', ')
          }));
        }
        return parseResult.data;
  ...
  const processInvoiceTextTemplateCSV = async (text: string) => {
    try {
      ...
      const parsed = dataRows.map((row) => {
        ...
        if (!parseResult.success) {
          throw new Error(t('admin:data_portability.invalid_row_invoice_text', {
            defaultValue: 'Ungültige Daten in Zeile für Rechnungstext-Vorlage: {{details}}',
            details: parseResult.error.issues.map(e => `${e.path.join('.')}: ${e.message}`).join(', ')
          }));
        }
        return parseResult.data;
  ...
  const processInvoiceItemTemplateCSV = async (text: string) => {
    try {
      ...
      const parsed = dataRows.map((row) => {
        ...
        const uScope = String(obj.usage_scope || '').toLowerCase().trim();
        obj.usage_scope = (uScope === 'offer' || uScope === 'invoice' || uScope === 'both') ? uScope : 'both';
        
        const parseResult = InvoiceItemTemplateSchema.partial({ tenant_id: true }).safeParse(obj);
        if (!parseResult.success) {
          throw new Error(t('admin:data_portability.invalid_row_invoice_item', {
            defaultValue: 'Ungültige Daten in Zeile für Rechnungsartikel-Vorlage: {{details}}',
            details: parseResult.error.issues.map(e => `${e.path.join('.')}: ${e.message}`).join(', ')
          }));
        }
        return parseResult.data;
      ...
      setInvoiceItemStatus({
        ...
        message: t('admin:data_portability.import_success_items', {
          defaultValue: '{{importedCount}} Angebots- & Rechnungsposten neu importiert, {{updatedCount}} Angebots- & Rechnungsposten aktualisiert.',
          importedCount: res.importedCount,
          updatedCount: res.updatedCount
        })
      });
  ...
  const processOfferTemplateCSV = async (text: string) => {
    try {
      setOfferTemplateStatus({ type: 'idle', message: t('admin:data_portability.parsing_csv', { defaultValue: 'Analysiere CSV...' }) });
      const rows = parseCSV(text);
      if (rows.length < 2) {
        throw new Error(t('admin:data_portability.csv_empty', { defaultValue: 'Die CSV-Datei ist leer oder enthaelt keine Datenzeilen.' }));
      }
      ...
      const parsed = dataRows.map((row) => {
        ...
        return {
          id_uuid: obj.id_uuid,
          template_name_text: obj.template_name_text || t('admin:data_portability.template_intro_default', { defaultValue: 'Standard Einleitung' }),
          template_type_code: obj.template_type_code || 'introductory',
          template_body_content: obj.template_body_content || ''
        };
      });

      const res = await importOfferTemplatesMutation.mutateAsync(parsed);
      setOfferTemplateStatus({
        type: 'success',
        message: t('admin:data_portability.import_success_offers', {
          defaultValue: '{{importedCount}} Angebotsvorlagen neu importiert, {{updatedCount}} Angebotsvorlagen aktualisiert.',
          importedCount: res.importedCount,
          updatedCount: res.updatedCount
        })
      });
  ...
  const analyzeCompaniesImport = (parsedRows: CompanyInput[]) => {
    const conflicts: CompanyConflict[] = [];
    const safe: CompanyInput[] = [];
  ...
  const analyzeContactsImport = (parsedRows: ContactInput[]) => {
    ...
    for (const incoming of parsedRows) {
      ...
      if (!matchedExisting) {
        ...
        matchedExisting = contacts.find(c => {
          const cPhone = normalizePhone(c.phone_number);
          const cMobile = normalizePhone(c.mobile_number);
    ...
    try {
      ...
      const parsedCompanies = dataRows.map((row) => {
        ...
        if (!rawObj.full_legal_name) {
          throw new Error(t('admin:data_portability.error_companyname_empty', { defaultValue: 'Spalte "full_legal_name" darf nicht leer sein.' }));
        }

        const parsed = CompanySchema.safeParse(rawObj);
        if (!parsed.success) {
          throw new Error(t('admin:data_portability.error_company_validation', {
            defaultValue: 'Zentrierte Validierung für {{name}} fehlgeschlagen: {{details}}',
            name: String(rawObj.full_legal_name || 'Unbekannt'),
  ...
  const processContactCSV = async (text: string) => {
    try {
      ...
      const parsedContacts = dataRows.map((row) => {
        ...
        if (!rawObj.last_name) {
          throw new Error(t('admin:data_portability.error_lastname_empty', { defaultValue: 'Spalte "last_name" darf nicht leer sein.' }));
        }

        const parsed = ContactSchema.safeParse(rawObj);
        if (!parsed.success) {
          throw new Error(t('admin:data_portability.error_contact_validation', {
            defaultValue: 'Zentrierte Validierung für {{name}} fehlgeschlagen: {{details}}',
            name: `${rawObj.first_name || ''} ${rawObj.last_name || 'Unbekannt'}`.trim(),
            details: parsed.error.message
          }));
        }
        return parsed.data;
  ...
  const submitResolvedCompanies = async () => {
    try {
      ...
      const uploadPack: CompanyInput[] = [...safeCompanies];
      ...
      for (const item of companyConflicts) {
        ...
        const payload: CompanyInput = { ...item.incoming };
  ...
  const submitResolvedContacts = async () => {
    try {
      ...
      const uploadPack: ContactInput[] = [...safeContacts];
      ...
      for (const item of contactConflicts) {
        ...
        const payload: ContactInput = { ...item.incoming };
  ...
  const handleDragOfferTemplate = (e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (e.type === "dragenter" || e.type === "dragover") {
      setDragActiveOfferTemplate(true);
    } else if (e.type === "dragleave") {
      setDragActiveOfferTemplate(false);
    }
  };

  const handleDropOfferTemplate = (e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setDragActiveOfferTemplate(false);
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      ...
      reader.onload = (event) => {
        if (event.target?.result) {
          processOfferTemplateCSV(event.target.result as string);
        }
      };
      reader.readAsText(file);
    }
  ...
  const handleFileOfferTemplateSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      const file = e.target.files[0];
      const reader = new FileReader();
      reader.onload = (event) => {
      ...
      {activeView === 'upload' && (
        <div className="space-y-12">
          ...
          <div className="flex border-b border-white/5 gap-6 pb-px">
            ...
            >
              {t('admin:data_portability.tabs.vorlagen', { defaultValue: 'E-Mail- & Signaturvorlagen' })}
            </button>
            <button
              id="tab-rechnungstexte"
              onClick={() => setActivePortabilityTab('rechnungstexte')}
              className={`pb-4 px-2 font-black text-xs uppercase tracking-widest border-b-2 transition-all cursor-pointer ${
                activePortabilityTab === 'rechnungstexte'
                  ? 'border-accent-orange text-white'
                  : 'border-transparent text-slate-500 hover:text-slate-300'
              }`}
            >
              {t('admin:data_portability.tabs.rechnungstexte', { defaultValue: 'Rechnungstexte' })}
            </button>
            <button
            ...
            >
              {t('admin:data_portability.tabs.rechnungsposten', { defaultValue: 'Angebots- & Rechnungsposten' })}
            </button>
            <button
              id="tab-angebotsvorlagen"
              onClick={() => setActivePortabilityTab('angebotstexte')}
              className={`pb-4 px-2 font-black text-xs uppercase tracking-widest border-b-2 transition-all cursor-pointer ${
                activePortabilityTab === 'angebotstexte'
                  ? 'border-accent-orange text-white'
                  : 'border-transparent text-slate-500 hover:text-slate-300'
              }`}
            >
              {t('admin:data_portability.tabs.angebotstexte', { defaultValue: 'Angebotstexte' })}
            </button>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-start">
                  ...
                  >
                    <Download size={12} />
                    {t('admin:data_portability.download_template_with_filename', { defaultValue: 'Download Vorlage ({{filename}})', filename: 'companies_template.csv' })}
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-start">
                  ...
                  >
                    <Download size={12} />
                    {t('admin:data_portability.download_template_with_filename', { defaultValue: 'Download Vorlage ({{filename}})', filename: 'contacts_template.csv' })}
                  </button>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-6 border border-white/5 p-5 rounded-2xl bg-primary-light/10">
                ...
                <div className="flex justify-start">
                  ...
                  >
                    <Download size={10} />
                    {t('admin:data_portability.download_template_with_filename', { defaultValue: 'Download Vorlage ({{filename}})', filename: 'emails_template.csv' })}
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-6 border border-white/5 p-5 rounded-2xl bg-primary-light/10">
                ...
                <div className="flex justify-start">
                  ...
                  >
                    <Download size={10} />
                    {t('admin:data_portability.download_template_with_filename', { defaultValue: 'Download Vorlage ({{filename}})', filename: 'signatures_template.csv' })}
                  </button>
                </div>
              </div>
          ...
          {activePortabilityTab === 'rechnungstexte' && (
            <div className="max-w-2xl mx-auto space-y-6 border border-white/5 p-6 rounded-2xl bg-primary-light/10">
              <div className="flex justify-between items-center bg-primary-dark/50 p-4 rounded-xl border border-white/5">
                <div className="flex items-center gap-3">
                  <FileSpreadsheet className="text-accent-orange" size={24} />
                  <h3 className="text-base font-black text-white uppercase tracking-tight">{t('admin:data_portability.invoice_texts', { defaultValue: 'Rechnungstexte' })}</h3>
                </div>
                <button
                ...
                >
                  <Download size={14} className="text-accent-orange" />
                  {t('admin:data_portability.export_csv', { defaultValue: 'CSV Exportieren' })} ({invoiceTextTemplates.length})
                </button>
              </div>
              ...
              >
                <UploadCloud size={36} className="text-slate-600 mb-4" />
                <p className="text-xs font-bold text-slate-400 mb-1">{t('admin:data_portability.invoice_texts_drag_and_drop', { defaultValue: 'Rechnungstexte-CSV hierher ziehen oder durchsuchen' })}</p>
                <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mb-4">{t('admin:data_portability.separator_hint', { defaultValue: 'Semicolon oder Comma getrennt' })}</p>
              ...
              {invoiceTextStatus.type !== 'idle' && (
                <div className={`p-4 rounded-xl flex items-start gap-3 text-xs font-bold ${
                  ...
                  {invoiceTextStatus.type === 'success' ? <CheckCircle size={16} /> : <AlertTriangle size={16} />}
              ...
              <div className="flex justify-start">
                ...
                >
                  <Download size={12} />
                  {t('admin:data_portability.download_template_with_filename', { defaultValue: 'Download Vorlage ({{filename}})', filename: 'invoice_texts_template.csv' })}
                </button>
              </div>
            </div>
          )}
          ...
          {activePortabilityTab === 'rechnungsposten' && (
            <div className="max-w-2xl mx-auto space-y-6 border border-white/5 p-6 rounded-2xl bg-primary-light/10">
              <div className="flex justify-between items-center bg-primary-dark/50 p-4 rounded-xl border border-white/5">
                <div className="flex items-center gap-3">
                  <FileSpreadsheet className="text-accent-orange" size={24} />
                  <h3 className="text-base font-black text-white uppercase tracking-tight">{t('admin:data_portability.invoice_items_catalog', { defaultValue: 'Angebots- & Rechnungsposten-Katalog' })}</h3>
                </div>
                <button
                  id="btn-export-items"
              ...
              >
                <UploadCloud size={36} className="text-slate-600 mb-4" />
                <p className="text-xs font-bold text-slate-400 mb-1">{t('admin:data_portability.drag_and_drop_items', { defaultValue: 'Angebots- & Rechnungsposten-CSV hierher ziehen oder durchsuchen' })}</p>
                <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mb-4">{t('admin:data_portability.separator_hint', { defaultValue: 'Semicolon oder Comma getrennt' })}</p>
              ...
              <div className="flex justify-start">
                ...
                >
                  <Download size={12} />
                  {t('admin:data_portability.download_template_with_filename', { defaultValue: 'Download Vorlage ({{filename}})', filename: 'invoice_item_templates_template.csv' })}
                </button>
              </div>
            </div>
          )}
          ...
          {activePortabilityTab === 'angebotstexte' && (
            <div className="max-w-2xl mx-auto space-y-6 border border-white/5 p-6 rounded-2xl bg-primary-light/10">
              <div className="flex justify-between items-center bg-primary-dark/50 p-4 rounded-xl border border-white/5">
                <div className="flex items-center gap-3">
                  <FileSpreadsheet className="text-accent-orange" size={24} />
                  <h3 className="text-base font-black text-white uppercase tracking-tight">{t('admin:data_portability.offer_texts', { defaultValue: 'Angebotstexte' })}</h3>
                </div>
                <button
                  id="btn-export-offers"
                  onClick={handleExportOfferTemplates}
                  className="flex items-center gap-2 bg-primary-light border border-white/10 hover:border-accent-orange/30 text-white text-[10px] font-black uppercase tracking-wider px-4 py-2.5 rounded-lg transition-all cursor-pointer"
                >
                  <Download size={14} className="text-accent-orange" />
                  {t('admin:data_portability.export_csv', { defaultValue: 'CSV Exportieren' })} ({offerTemplates.length})
                </button>
              </div>

              {/* Drag & Drop Area */}
              <div
                id="dropzone-offer-templates"
                onDragEnter={handleDragOfferTemplate}
                onDragOver={handleDragOfferTemplate}
                onDragLeave={handleDragOfferTemplate}
                onDrop={handleDropOfferTemplate}
                className={`relative border-2 border-dashed rounded-2xl py-12 px-6 flex flex-col items-center justify-center text-center transition-all ${
                  dragActiveOfferTemplate
                    ? 'border-accent-orange bg-accent-orange/5'
                    : 'border-white/5 bg-primary-light/10 hover:border-white/10'
                }`}
              >
                <UploadCloud size={36} className="text-slate-600 mb-4" />
                <p className="text-xs font-bold text-slate-400 mb-1">{t('admin:data_portability.drag_and_drop_offers', { defaultValue: 'Angebotstext-CSV hierher ziehen oder durchsuchen' })}</p>
                <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mb-4">{t('admin:data_portability.separator_hint', { defaultValue: 'Semicolon oder Comma getrennt' })}</p>
                ...
                <label className="cursor-pointer bg-primary-light hover:bg-primary-light/80 text-white font-black text-[9px] uppercase tracking-widest px-4 py-2 border border-white/15 rounded-lg transition-all">
                  ...
                  <input
                    type="file"
                    accept=".csv"
                    onChange={handleFileOfferTemplateSelect}
                    className="hidden"
                  />
                </label>
              </div>
              ...
              {/* Status Display */}
              {offerTemplateStatus.type !== 'idle' && (
                <div className={`p-4 rounded-xl flex items-start gap-3 text-xs font-bold ${
                  offerTemplateStatus.type === 'success' 
                    ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400' 
                    : 'bg-rose-500/10 border border-rose-500/20 text-rose-400'
                }`}>
                  {offerTemplateStatus.type === 'success' ? <CheckCircle size={16} /> : <AlertTriangle size={16} />}
                  <span>{offerTemplateStatus.message}</span>
                </div>
              )}

              <div className="flex justify-start">
                <button
                  id="btn-template-offers"
                  onClick={handleDownloadTemplateOfferTemplates}
                  className="text-slate-600 hover:text-slate-400 font-black uppercase text-[9px] tracking-widest flex items-center gap-1.5 transition-all cursor-pointer"
                >
                  <Download size={12} />
                  {t('admin:data_portability.download_template_with_filename', { defaultValue: 'Download Vorlage ({{filename}})', filename: 'offer_text_templates_template.csv' })}
                </button>
              </div>
            </div>
          )}
      ...
      {activeView === 'resolve_companies' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                ...
                <div key={item.id} className="border border-white/5 bg-primary-light/10 rounded-2xl overflow-hidden shadow-xl">
                  ...
                  <div className="bg-amber-500/10 border-b border-white/5 px-6 py-3 text-xs font-bold text-amber-300 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span>{t('admin:data_portability.entry_match', { index: index + 1 })}</span>
                    </div>
                  ...
                  <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8 bg-black/20">
                    ...
                    <div className="space-y-4">
                      <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                        ...
                        {t('admin:data_portability.new_csv_data')}
                      </h4>
                      <div className="space-y-3 bg-white/[0.02] border border-white/5 p-4 rounded-xl">
                        <div className={`p-2.5 rounded-lg ${diffName ? 'bg-accent-orange/10 border border-accent-orange/20' : ''}`}>
                          <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-0.5">{t('admin:data_portability.company_name')}</div>
                        ...
                        <div className={`p-2.5 rounded-lg ${diffTax ? 'bg-accent-orange/10 border border-accent-orange/20' : ''}`}>
                          <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-0.5">{t('admin:data_portability.vat_tax_number')}</div>
                        ...
                        <div className={`p-2.5 rounded-lg ${diffEmail ? 'bg-accent-orange/10 border border-accent-orange/20' : ''}`}>
                          <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-0.5">{t('admin:data_portability.email_address')}</div>
                        ...
                        <div className={`p-2.5 rounded-lg ${diffPhone ? 'bg-accent-orange/10 border border-accent-orange/20' : ''}`}>
                          <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-0.5">{t('common:fields.phone', { defaultValue: 'Telefonnummer' })}</div>
                        ...
                        <div className={`p-2.5 rounded-lg ${diffCity ? 'bg-accent-orange/10 border border-accent-orange/20' : ''}`}>
                          <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-0.5">{t('admin:data_portability.address_city')}</div>
                    ...
                    <div className="space-y-4">
                      <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                        ...
                        {t('admin:data_portability.existing_system_profile')}
                      </h4>
                      <div className="space-y-3 bg-white/[0.02] border border-white/5 p-4 rounded-xl">
                  ...
                  <div className="bg-primary-dark/80 border-t border-white/5 p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <span className="text-xs font-black uppercase text-slate-400 tracking-wider">{t('admin:data_portability.choose_action')}</span>
                    ...
                    <div className="grid grid-cols-3 gap-3 w-full md:w-auto">
                      ...
                      >
                        {t('admin:data_portability.action_update')}
                      </button>
                      ...
                      >
                        {t('admin:data_portability.action_create')}
                      </button>
                      ...
                      >
                        {t('admin:data_portability.action_discard')}
                      </button>
          ...
          <div className="flex justify-end gap-4 border-t border-white/5 pt-8">
            ...
            >
              <Check size={14} />
              {t('admin:data_portability.complete_import_and_import', { count: companyConflicts.length + safeCompanies.length })}
            </button>
          </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
          ...
          <div className="space-y-6">
            {contactConflicts.map((item, index) => {
              ...
              const incomingCompName = companies.find(c => c.id_uuid === item.incoming.associated_company_id)?.full_legal_name || t('admin:data_portability.no_linked_company');
              const existingCompName = companies.find(c => c.id_uuid === item.existing.associated_company_id)?.full_legal_name || t('admin:data_portability.no_linked_company');
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3 bg-white/[0.02] border border-white/5 p-4 rounded-xl">
                        <div className={`p-2.5 rounded-lg ${diffName ? 'bg-accent-blue/10 border border-accent-blue/20' : ''}`}>
                          <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-0.5">{t('admin:data_portability.person_contact')}</div>
                        ...
                        <div className={`p-2.5 rounded-lg ${diffCompany ? 'bg-accent-blue/10 border border-accent-blue/20' : ''}`}>
                          <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-0.5">{t('contacts:fields.entity_link', { defaultValue: 'Verknüpftes Unternehmen' })}</div>
                        ...
                        <div className={`p-2.5 rounded-lg ${diffPhone ? 'bg-accent-blue/10 border border-accent-blue/20' : ''}`}>
                          <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-0.5">{t('common:fields.phone', { defaultValue: 'Telefonnummer' })} / {t('contacts:fields.mobile', { defaultValue: 'Mobil' })}</div>
          ...
          <div className="flex justify-end gap-4 border-t border-white/5 pt-8">
            ...
            >
              <Check size={14} />
              {t('admin:data_portability.complete_import_and_import', { count: contactConflicts.length + safeContacts.length })}
            </button>
          </div>
