...
export const SmtpSettingsForm = () => {
  ...
  const saveSmtpMutation = trpc.saveSmtpSettings.useMutation({
    ...
    onError: (err) => {
      toast.error(t('smtp.error') + ': ' + formatValidationErrors(err.message, t));
    }
  });
  ...
  const testSmtpMutation = trpc.testSmtp.useMutation({
    ...
    onError: (err) => {
      toast.error(t('smtp.test_error') + ': ' + formatValidationErrors(err.message, t));
    }
  });
  ...
  const handleSmtpSave = (e: React.FormEvent) => {
    e.preventDefault();
    if (
      !smtpForm.smtp_host_name.trim() ||
      !smtpForm.smtp_user_name.trim() ||
      !smtpForm.smtp_password_secret.trim() ||
      !smtpForm.sender_email_address.trim() ||
      isNaN(smtpForm.smtp_port_number) ||
      smtpForm.smtp_port_number <= 0
    ) {
      ...
      <form onSubmit={handleSmtpSave} className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-2">
          ...
          <input 
            ...
            className={cn(
              "w-full bg-primary-dark/60 border rounded-xl px-6 py-4 text-white font-bold focus:outline-none transition-colors",
              isSubmitAttempted && !smtpForm.smtp_host_name.trim()
        ...
        <div className="space-y-2">
          ...
          <input 
            ...
            onChange={(e) => setSmtpForm({...smtpForm, smtp_port_number: Math.max(1, parseInt(e.target.value) || 587)})}
            placeholder="587"
            className={cn(
              "w-full bg-primary-dark/60 border rounded-xl px-6 py-4 text-white font-bold focus:outline-none transition-colors",
              isSubmitAttempted && (!smtpForm.smtp_port_number || smtpForm.smtp_port_number <= 0)
                ? "border-red-500/50 focus:border-red-500"
                : "border-white/10 focus:border-accent-blue"
            )}
            required
        ...
        <div className="space-y-2">
          ...
          <input 
            ...
            className={cn(
              "w-full bg-primary-dark/60 border rounded-xl px-6 py-4 text-white font-bold focus:outline-none transition-colors",
              isSubmitAttempted && !smtpForm.smtp_user_name.trim()
                ? "border-red-500/50 focus:border-red-500"
                : "border-white/10 focus:border-accent-blue"
            )}
            required
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <input 
              ...
              className={cn(
                "w-full bg-primary-dark/60 border rounded-xl px-6 py-4 text-white font-bold focus:outline-none transition-colors pr-12",
                isSubmitAttempted && !smtpForm.smtp_password_secret.trim()
                  ? "border-red-500/50 focus:border-red-500"
                  : "border-white/10 focus:border-accent-blue"
              )}
              required
        ...
        <div className="space-y-2">
          ...
          <input 
            ...
            className={cn(
              "w-full bg-primary-dark/60 border rounded-xl px-6 py-4 text-white font-bold focus:outline-none transition-colors",
              isSubmitAttempted && !smtpForm.sender_email_address.trim()
                ? "border-red-500/50 focus:border-red-500"
                : "border-white/10 focus:border-accent-blue"
            )}
            required
