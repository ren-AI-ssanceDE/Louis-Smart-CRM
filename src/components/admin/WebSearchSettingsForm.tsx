...
export const WebSearchSettingsForm = () => {
  ...
  const saveSearchMutation = trpc.saveWebSearchSettings.useMutation({
    ...
    onError: (err) => {
      toast.error(t('admin:toast_error_web_search', { defaultValue: 'Fehler beim Speichern der Websuche-Parameter: ' }) + formatValidationErrors(err.message, t));
    }
  });

  useEffect(() => {
    if (searchData) {
      setSelectedEngine(searchData.selected_engine as 'duckduckgo' | 'searxng' | 'google_grounding' | 'google_custom_search');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Contextual required field validation
    let isValid = true;
    if (selectedEngine === 'duckduckgo' && !duckduckgoUrl.trim()) {
      isValid = false;
    } else if (selectedEngine === 'searxng' && !searxngUrl.trim()) {
      isValid = false;
    } else if (selectedEngine === 'google_custom_search' && (!googleApiKey.trim() || !googleCx.trim())) {
      isValid = false;
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-2 col-span-2 md:col-span-1">
            ...
            <select
              ...
              onChange={(e) => setSelectedEngine(e.target.value as 'duckduckgo' | 'searxng' | 'google_grounding' | 'google_custom_search')}
            ...
            {selectedEngine === 'duckduckgo' && (
              <>
                ...
                <input
                  ...
                  placeholder={t('admin:search_settings.ddg_url_placeholder', { defaultValue: 'https://html.duckduckgo.com/html/' })}
                  className={cn(
                    "w-full bg-primary-dark/60 border rounded-xl px-6 py-4 text-white font-bold focus:outline-none transition-colors font-mono",
                    isSubmitAttempted && !duckduckgoUrl.trim()
            ...
            {selectedEngine === 'searxng' && (
              <>
                ...
                <input
                  ...
                  placeholder={t('admin:search_settings.searxng_url_placeholder', { defaultValue: 'https://searxng.org/search' })}
                  className={cn(
                    "w-full bg-primary-dark/60 border rounded-xl px-6 py-4 text-white font-bold focus:outline-none transition-colors font-mono",
                    isSubmitAttempted && !searxngUrl.trim()
                      ? "border-red-500/50 focus:border-red-500"
                      : "border-white/10 focus:border-accent-blue"
                  )}
                  required
            ...
            <>
              ...
              <div className="space-y-2 col-span-2 md:col-span-1">
                ...
                <input
                  ...
                  className={cn(
                    "w-full bg-primary-dark/60 border rounded-xl px-6 py-4 text-white font-bold focus:outline-none transition-colors font-mono",
                    isSubmitAttempted && !googleApiKey.trim()
                      ? "border-red-500/50 focus:border-red-500"
                      : "border-white/10 focus:border-accent-blue"
                  )}
                  required
              ...
              <div className="space-y-2 col-span-2 md:col-span-1">
                ...
                <input
                  ...
                  className={cn(
                    "w-full bg-primary-dark/60 border rounded-xl px-6 py-4 text-white font-bold focus:outline-none transition-colors font-mono",
                    isSubmitAttempted && !googleCx.trim()
                      ? "border-red-500/50 focus:border-red-500"
                      : "border-white/10 focus:border-accent-blue"
                  )}
                  required
