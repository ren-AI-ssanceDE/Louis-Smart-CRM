...
import { Bot, Loader2, CheckCircle2, AlertCircle, Send, ShieldAlert, Lock, Trash2, Plus, ChevronDown, ChevronUp, Cpu } from 'lucide-react';
...
export const TelegramSettingsForm = () => {
  ...
  const saveTelegramMutation = trpc.saveTelegramSettings.useMutation({
    ...
    onError: (err) => {
      toast.error(t('admin:toast_error_telegram', { defaultValue: 'Fehler beim Speichern der Telegram-Parameter: ' }) + formatValidationErrors(err.message, t));
    }
  });
  ...
  const testTelegramMutation = trpc.testTelegramConnection.useMutation({
    ...
    onError: (err) => {
      setTestResult({
        success: false,
        message: formatValidationErrors(err.message, t)
      });
      toast.error(formatValidationErrors(err.message, t));
    }
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!botToken.trim() || !allowedUserIds.trim()) {
  ...
  return (
    <div className="bg-primary-dark/40 border border-white/5 rounded-xl p-10 space-y-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-2 col-span-2">
            ...
            <div className="relative">
              <input
                ...
                placeholder={t('admin:telegram_settings.bot_token_placeholder', { defaultValue: '0123456789:ABCdefGhIJKlmNoPQRsTUVwxyZ...' })}
                className={cn(
                  "w-full bg-primary-dark/60 border rounded-xl pl-12 pr-6 py-4 text-white font-mono text-sm focus:outline-none transition-colors",
                  isSubmitAttempted && !botToken.trim()
                    ? "border-red-500/50 focus:border-accent-orange"
          ...
          <div className="space-y-2 col-span-2">
            ...
            <input
              ...
              className={cn(
                "w-full bg-primary-dark/60 border rounded-xl px-6 py-4 text-white font-mono text-sm focus:outline-none transition-colors",
                isSubmitAttempted && !allowedUserIds.trim()
                  ? "border-red-500/50 focus:border-accent-orange"
                  : "border-white/10 focus:border-accent-orange"
              )}
              required
        ...
        {/* Collapsible Network PC Guide */}
        <div className="border border-white/5 rounded-xl overflow-hidden bg-primary-dark/10">
          <button
            type="button"
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-white font-display">
                {t('admin:telegram_settings.toggle_network_info', { defaultValue: 'Netzwerk-Betriebsanleitung einblenden / ausblenden' })}
              </span>
            </div>
            {showNetworkInfo ? <ChevronUp size={16} className="text-slate-400" /> : <ChevronDown size={16} className="text-slate-400" />}
          </button>

          {showNetworkInfo && (
            <div className="p-6 border-t border-white/5 space-y-4 text-xs text-slate-300 leading-relaxed font-sans">
              <h5 className="font-bold text-white text-[11px] uppercase tracking-wider font-display">
                {t('admin:telegram_settings.network_setup_title', { defaultValue: 'Separater Netzwerk-PC & Docker-Betrieb' })}
              </h5>
              <p className="text-slate-400 text-[10px] uppercase tracking-wider font-bold">
                {t('admin:telegram_settings.network_setup_desc', { defaultValue: 'Falls das Telegram Gateway oder der Whisper-Server auf einem separaten Rechner im lokalen Netzwerk läuft:' })}
              </p>

              <div className="space-y-3 bg-black/30 rounded-xl p-5 border border-white/5">
                <p className="font-medium text-slate-300">
                  {t('admin:telegram_settings.network_setup_stt_desc', { defaultValue: '1. Whisper-Server im Netzwerk: Tragen Sie einfach die IP-Adresse Ihres Netzwerk-PCs im Admin-Panel unter \'Louis AI Einstellungen\' ein (z. B. http://192.168.1.100:8000/v1/audio/transcriptions).' })}
                </p>

                <div className="border-t border-white/5 pt-3">
                  <p className="font-medium text-slate-300 mb-2">
                    {t('admin:telegram_settings.network_setup_tg_desc', { defaultValue: '2. Telegram Bot im Netzwerk: Der Bot läuft als leichter Docker-Container auf dem anderen PC. Er benötigt dort KEINE Token- oder ID-Konfigurationen in der .env. Er muss lediglich wissen, wie das CRM erreichbar ist:' })}
                  </p>
                  <p className="text-[10px] font-black uppercase text-accent-orange/80 tracking-widest block mb-2">
                    {t('admin:telegram_settings.network_setup_env_lbl', { defaultValue: 'Starten Sie den Bot-Container auf dem Netzwerk-PC einfach mit dieser einzigen Umgebungsvariable:' })}
                  </p>
                  
                  <div className="bg-primary-dark/80 rounded-lg p-3 font-mono text-[11px] text-white overflow-x-auto border border-white/5">
                    <span className="text-slate-400">docker run -d \</span><br />
                    &nbsp;&nbsp;<span className="text-slate-400">-e</span> CRM_CONFIG_API=&quot;{typeof window !== 'undefined' ? window.location.origin : 'http://YOUR_CRM_IP:3000'}/api/telegram/config&quot; \<br />
                    &nbsp;&nbsp;<span className="text-slate-400">--name</span>{" louis-telegram-gate \\"}<br />
                    &nbsp;&nbsp;telegram-bot-gate
                  </div>
                </div>
              </div>
