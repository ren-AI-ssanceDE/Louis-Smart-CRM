...
export const Sidebar = ({ activeTab, setActiveTab, timezone, isLouisAiOpen, setIsLouisAiOpen }: SidebarProps) => {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Set the explicit_logout cookie so the server knows the user explicitly signed out
      document.cookie = "explicit_logout=1; path=/; max-age=31536000";
  ...
  const [menuItems, setMenuItems] = React.useState<MenuItem[]>([
    ...
    { id: 'offers', icon: FileSpreadsheet, label: t('offers:title') || 'Angebote' },
    { id: 'kanban', icon: KanbanSquare, label: 'Kanban' },
    { id: 'council', icon: Users, label: 'LLM Council' },
