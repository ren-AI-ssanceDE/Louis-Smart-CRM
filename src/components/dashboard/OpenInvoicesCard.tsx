...
export const OpenInvoicesCard: React.FC<OpenInvoicesCardProps> = ({ onNavigate }) => {
  ...
  // Filter open (issued or overdue) invoices
  const openInvoices = invoices.filter(
    (inv) => inv.payment_status === 'issued' || inv.payment_status === 'overdue'
  );
  ...
  const isOverdueByDate = (inv: { payment_status?: string | null; due_date?: string | null }) => {
    ...
    if (inv.payment_status === 'issued' && inv.due_date) {
        ...
        {isLoading ? (
          ...
          <div className="space-y-3 mb-6">
            {sortedOpenInvoices.map((inv) => {
              ...
              const recipientName = inv.company_name || inv.contact_full_name || t('contacts.unknown_fallback', { defaultValue: 'Unbekannter Empfänger' });
