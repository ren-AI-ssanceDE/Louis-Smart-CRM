...
export const MailDialog = ({ 
  ...
  const profileType = associatedType || (invoice?.associated_contact_id ? 'contacts' : (invoice?.associated_company_id || offer?.associated_company_id) ? 'companies' : offer?.associated_contact_id ? 'contacts' : undefined);
  const profileId = associatedId || invoice?.associated_contact_id || invoice?.associated_company_id || offer?.associated_company_id || offer?.associated_contact_id;
  ...
  const replacePlaceholders = (text: string, companyName: string, contactPerson: string) => {
    ...
    if (profileType === 'companies' && profileId) {
      const co = companyList.find((c) => c.id_uuid === profileId);
      if (co) {
      ...
      const ct = contactList.find((c) => c.id_uuid === profileId);
      if (ct) {
        ...
        if (ct.company_name) {
          ...
          const assocCo = companyList.find((co) => co.id_uuid === ct.associated_company_id);
          if (assocCo) {
    ...
    if (!rStreet && !rCity) {
      const foundCt = contactList.find((c) => (c.email_address && c.email_address.toLowerCase() === recipientEmail.toLowerCase()) || c.full_legal_name === recipientName);
      if (foundCt) {
        ...
        if (foundCt.company_name) {
          ...
          const assocCo = companyList.find((co) => co.id_uuid === foundCt.associated_company_id);
          if (assocCo) {
            rCompany = assocCo.full_legal_name || '';
          }
        }
        ...
        const foundCo = companyList.find((c) => (c.email_address && c.email_address.toLowerCase() === recipientEmail.toLowerCase()) || c.full_legal_name === recipientName);
        if (foundCo) {
    ...
    if (invoice) {
      ...
      const validUntilDateObj = new Date(offer.valid_until);
      const activeLocale = i18n.language || 'de';
      const validUntilStr = validUntilDateObj.toLocaleDateString(activeLocale);
      const grossVal = typeof offer.total_gross_amount === 'number' 
        ? offer.total_gross_amount.toLocaleString(activeLocale, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
        : (0).toLocaleString(activeLocale, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      ...
      replaced = replaced
        ...
        .replace(/\{\{total_gross\}\}/g, grossVal)
        .replace(/\{\{currency\}\}/g, offer.currency_code || 'EUR');
    } else {
      replaced = replaced
        .replace(/\{\{invoice_number\}\}/g, 'RE-2024-001')
        .replace(/\{\{due_date\}\}/g, '31.05.2026')
        .replace(/\{\{offer_number\}\}/g, 'AG-2024-001')
        .replace(/\{\{valid_until\}\}/g, '31.05.2026')
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center gap-4 p-4 bg-amber-500/10 rounded-xl border border-amber-500/20">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs font-bold text-amber-400 truncate">
                {`${t('offers:offer_single', { defaultValue: 'Angebot' })} - ${recipientName.replace(/[/\\?%*:|"<>\.]/g, '')} - ${offer.offer_number.replace(/[^a-zA-Z0-9_-]/g, '_')}.pdf`}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between ml-2">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest font-display flex items-center gap-1.5">
            ...
            <div className="flex bg-primary-dark/85 p-0.5 rounded-lg border border-white/5 gap-0.5">
              ...
              >
                {t('admin:templates.editor_mode_edit')}
              </button>
              <button
                type="button"
              ...
              >
                {t('admin:templates.editor_mode_preview')}
              </button>
            </div>
          </div>
 ...
 // : Editor ist nie mehr mit Signatur vorbelegt (kein Prefill) —
                  ...
                  className="p-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 hover:text-emerald-300 rounded-lg transition-colors flex items-center gap-1.5 ml-auto text-[10px] font-black uppercase tracking-widest font-display cursor-pointer"
            ...
            <div className="w-full bg-slate-100 border border-slate-300 rounded-xl overflow-hidden p-6 text-slate-900 font-sans shadow-inner animate-fade-in">
              <div className="pb-4 mb-4 border-b border-slate-300 text-xs text-slate-500 space-y-1">
                <div className="flex justify-between items-center bg-transparent">
                  <span><strong className="text-slate-700">{t('admin:templates.live_preview_sender')}:</strong> Louis Smart CRM &lt;no-reply@louis-crm.de&gt;</span>
                  <span className="text-[9px] bg-accent-blue text-white font-sans font-black tracking-widest px-2.5 py-1 rounded-full uppercase">{t('admin:templates.live_preview_label')}</span>
                </div>
                <div><strong className="text-slate-700">{t('admin:templates.live_preview_recipient')}:</strong> {recipientName || <span className="italic text-slate-400">---</span>} &lt;{recipientEmail || '---'}&gt;</div>
                <div><strong className="text-slate-700">{t('admin:templates.live_preview_subject')}:</strong> {subject || <span className="italic text-slate-400">({t('admin:mail.subject_placeholder')})</span>}</div>
              </div>

              {/* Template Email Layout Simulation */}
              <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 h-[350px] overflow-y-auto custom-scrollbar shadow-sm">
                <div className="max-w-[500px] mx-auto bg-white rounded-lg border border-slate-200 overflow-hidden">
                  {/* Body only (Header and Footer and Greeting removed) */}
                  <div className="p-6">
                    {body ? (
                      <div 
                        className="email-preview-content font-sans text-xs text-slate-700 leading-relaxed [&_p]:m-0"
                        dangerouslySetInnerHTML={{ __html: body }}
                      />
                    ) : (
                      <p className="text-slate-400 italic text-xs">{t('admin:templates.live_preview_empty')}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 flex items-start gap-2 text-[10px] text-slate-600 bg-slate-200/50 p-4 rounded-lg border border-slate-300/40 leading-normal">
                ...
                <span>{t('admin:templates.live_preview_info')}</span>
              </div>
            </div>
          )}
        </div>
