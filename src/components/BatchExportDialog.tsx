...
export const BatchExportDialog: React.FC<BatchExportDialogProps> = ({
  ...
  const eligibleInvoices = useMemo(() => {
    return invoices.filter(invoice => {
      ...
      const isAfterOrEqual = invoiceMonth >= startMonth;
      const isBeforeOrEqual = invoiceMonth <= endMonth;
  ...
  const handleExport = async () => {
    ...
    try {
      for (let i = 0; i < eligibleInvoices.length; i++) {
        ...
        const crmRecipientLabel = t('common:crm_recipient', { defaultValue: 'CRM-Empfänger' });
        setCurrentLabel(`${invoice.invoice_number} (${invoice.company_name || invoice.contact_full_name || crmRecipientLabel})`);
        ...
        const defaultRecipient = t('common:recipient', { defaultValue: 'Empfänger' });
        const recipientName = company?.full_legal_name || (contact ? `${contact.first_name || ''} ${contact.last_name || ''}`.trim() : '') || invoice.company_name || defaultRecipient;
        ...
        if (exportType === 'pdf' || exportType === 'both') {
          try {
            ...
            if (res.ok) {
              ...
              const invoiceLabel = t('common:invoice_single', { defaultValue: 'Rechnung' });
              const pdfFilename = `${invoiceLabel} - ${cleanRecipient} - ${cleanNum}.pdf`;
              if (pdfFolder) {
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => setExportType(opt.value as 'pdf' | 'xml' | 'both')}
                  ...
                  <>
                    ...
                    <span>
                      {t('batch_export_matching_text', { 
                        defaultValue: 'Es wurden \x3cstrong\x3e{{count}}\x3c/strong\x3e exportierbare Rechnungen für den Zeitraum \x3cstrong\x3e{{start}}\x3c/strong\x3e bis \x3cstrong\x3e{{end}}\x3c/strong\x3e gefunden.', 
