        ...
        for (const update of body.result) {
          ...
          if (update.message) {
            if (update.message.text) {
              ...
              handleIncomingVoiceMessage(update.message).catch(e => {
                console.error("[Telegram API] Failed to handle voice message:", e);
              });
            }
          }
        }
...
async function handleIncomingVoiceMessage(message) {
  const chatId = message.chat.id;
  const userId = message.from?.id;
  const voice = message.voice;

  console.log(`[Voice Message] Received from Chat ${chatId} / User ID ${userId}, File ID: ${voice.file_id}`);
  ...
  await sendTelegramAction(chatId, "record_audio");

  try {
    const activeToken = currentBotToken;
    // 1. Get the file path from Telegram
    const fileInfoRes = await fetch(`https://api.telegram.org/bot${activeToken}/getFile?file_id=${voice.file_id}`);
    if (!fileInfoRes.ok) {
      throw new Error(`Telegram getFile endpoint failed with status: ${fileInfoRes.status}`);
    }
    const fileInfo = await fileInfoRes.json();
    if (!fileInfo.ok || !fileInfo.result?.file_path) {
      throw new Error("Could not retrieve file path from Telegram API response.");
    }

    const filePath = fileInfo.result.file_path;
    const fileUrl = `https://api.telegram.org/file/bot${activeToken}/${filePath}`;

    // 2. Download the voice file (ogg/opus)
    console.log(`[Voice Message] Downloading file from Telegram...`);
    ...
    // 3. Transcribe via Louis STT Router
    await sendTelegramAction(chatId, "typing");
    await sendTelegramMessage(chatId, "🎙️ _Audionachricht empfangen. Transkribiere via Whisper..._");

    // Match CRM base URL from CRM_CONFIG_API
    const crmBaseUrl = CRM_CONFIG_API.replace("/api/telegram/config", "");
    const transcribeUrl = `${crmBaseUrl}/api/voice/transcribe`;

    console.log(`[Voice Message] Uploading voice stream to Louis CRM STT at: ${transcribeUrl}`);
    
    const formData = new FormData();
    const blob = new Blob([buffer], { type: "audio/ogg" });
    formData.append("file", blob, "telegram_voice.ogg");
    ...
    if (!transcribeRes.ok) {
      const errTxt = await transcribeRes.text().catch(() => "Unknown error");
      throw new Error(`Whisper transcription server failed, status: ${transcribeRes.status}, error: ${errTxt}`);
    }

    const transcribeData = await transcribeRes.json();
    if (!transcribeData || !transcribeData.text) {
      throw new Error("Transkription lieferte leeren Text.");
    }

    const textResult = transcribeData.text.trim();
    console.log(`[Voice Message] Successfully Transcribed: "${textResult}"`);

    await sendTelegramMessage(chatId, `📝 *Transkript:* _"${textResult}"_\n\n_Verarbeite Anfrage..._`);

    // 4. Inject transcribe text back to standard handler
    const mockMessage = { ...message, text: textResult };
    await handleIncomingMessage(mockMessage);

  } catch (err) {
    console.error("Failed to process Telegram voice message:", err);
    await sendTelegramMessage(chatId, `⚠️ *Transkriptionsfehler:* ${err.message || "Audiodatei konnte nicht verarbeitet werden."}`);
  }
}
