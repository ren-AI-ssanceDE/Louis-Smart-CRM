...
- **E-Rechnung**: ZUGFeRD/XRechnung als PDF/A-3 mit Mustang-Validierungs-Gate und Ghostscript-Normalisierung (kritische Systemdatei, produktiv abgesichert).
...
- **Workflow-Notiz wird zuverlässig persistiert:** `create_note_draft`-Schritte im linearen Workflow-Executor und im DAG-Executor (Dispatch `executeCreateNoteDraft` + LLM-Dispatch `CreateNote`) übergeben `bypassApproval=true` — vorher liefen alle drei Pfade als No-Op (nur Draft-Meldung bzw. nur Audit ohne DB-Eintrag). Verifiziert: Notiz landet in der Datenbank (alle Workflow-Pfade).
...
- Tenant-List-Vektor (`OR tenant_id = '1'`-Muster in Listen-Tools) — in Arbeit
- Lokaler-LLM-Subtask-Pfad unter paralleler LLM-Last (Timeout) — in Arbeit
