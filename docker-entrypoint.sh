...
if [ "$(id -u)" = '0' ]; then
    ...
    mkdir -p /app/companies_data_vault /app/contacts_data_vault /app/knowledge_data_vault /app/uploads
    
    # Change ownership of persistent volume mounts to the non-root 'app' user
    chown -R app:app /app/companies_data_vault /app/contacts_data_vault /app/knowledge_data_vault /app/uploads
